...
let as_prod ?(penv=Bindlib.empty_ctxt) = function
  | TT.(Ty (Prod (u, t))) when Bindlib.binder_occur t ->
     let (x, t, penv) = Bindlib.unbind_in penv t in
     Some (x, u, t, penv)
  | _ -> None

let as_lambda ?(penv=Bindlib.empty_ctxt) = function
  | TT.Lambda (t, e) ->
     let (x, e, penv) = Bindlib.unbind_in penv e in
     Some (x, t, e, penv)
  | _ -> None

let rec tm ?max_level ?(penv=Bindlib.empty_ctxt) e ppf =
  match e with
...
and ty ?max_level ?(penv=Bindlib.empty_ctxt) (Ty t) ppf =
  tm ?max_level ~penv t ppf
